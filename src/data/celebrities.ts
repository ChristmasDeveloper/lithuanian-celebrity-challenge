...
const hiddenImageModules = import.meta.glob(
  "/public/images/hidden/*.{png,jpg,jpeg,webp,gif}",
  { eager: true, as: "url" },
);
...
export const checkGuess = (guess: string, celebrity: Celebrity): boolean => {
  ...
  const reverseName = normalizeString(
    `${celebrity.lastname} ${celebrity.name}`,
  );
  ...
  const nameWords = `${celebrity.name} ${celebrity.lastname}`
    .split(" ")
    .map(normalizeString);
